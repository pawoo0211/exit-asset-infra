...
MODE="${2:-foreground}"
PID_FILE="/tmp/mongodb-port-forward-${LOCAL_PORT}.pid"
LOG_FILE="/tmp/mongodb-port-forward-${LOCAL_PORT}.log"

if ! kubectl get svc "$SERVICE" -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "Service $SERVICE not found in namespace $NAMESPACE"
  echo "Run: ./mongodb/scripts/deploy.sh"
  exit 1
fi

if lsof -nP -iTCP:"$LOCAL_PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" >/dev/null 2>&1; then
    echo "MongoDB port-forward already running on localhost:$LOCAL_PORT"
    echo "PID: $(cat "$PID_FILE")"
    exit 0
  fi
  echo "Port $LOCAL_PORT is already in use."
  echo "Try: ./mongodb/scripts/port-forward.sh 27018"
  exit 1
fi
...
if [[ "$MODE" == "--background" ]]; then
  ...
  echo "Failed to start MongoDB port-forward. Check log: $LOG_FILE"
  exit 1
fi

...
MODE="${2:-foreground}"
PID_FILE="/tmp/argocd-port-forward-${LOCAL_PORT}.pid"
LOG_FILE="/tmp/argocd-port-forward-${LOCAL_PORT}.log"

if ! kubectl get svc "$SERVICE" -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "Service $SERVICE not found in namespace $NAMESPACE"
  echo "Run: ./argocd/scripts/deploy.sh"
  exit 1
fi

if lsof -nP -iTCP:"$LOCAL_PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" >/dev/null 2>&1; then
    echo "Argo CD port-forward already running on http://localhost:$LOCAL_PORT"
    echo "PID: $(cat "$PID_FILE")"
    exit 0
  fi
  echo "Port $LOCAL_PORT is already in use."
  echo "Try: ./argocd/scripts/port-forward.sh 18093"
  exit 1
fi
...
if [[ "$MODE" == "--background" ]]; then
  ...
  echo "Failed to start Argo CD port-forward. Check log: $LOG_FILE"
  exit 1
fi
